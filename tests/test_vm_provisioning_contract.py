...
class VmProvisioningContractTests(unittest.TestCase):
    ...
    def test_host_smoke_orchestrator_isolated_and_credential_scoped(self):
        ...
        self.assertIn("PowerShell.exe -NoProfile -ExecutionPolicy Bypass -File $isolationScript", source)
        self.assertIn("$isolationExitCode = [int]$LASTEXITCODE", source)
        self.assertIn("PowerShell.exe -NoProfile -ExecutionPolicy Bypass -File $stageScript", source)
        self.assertIn("$stageExitCode = [int]$LASTEXITCODE", source)
        self.assertIn("Get-Credential", source)
        self.assertIn("Invoke-Command", source)
        self.assertIn("-VMName $VmName", source)
