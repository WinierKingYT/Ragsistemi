...
| Signed evidence packaging | `scripts/build_signed_evidence.py` signs only strict, explicitly supplied EXT-01..EXT-06 or FA-02..FA-06 observations, self-verifies the output, refuses overwrite and rejects final packaging against blocked readiness |
...
- A clean isolated exact-lock validation environment is materialized under
  ...
  pipeline completes the full suite with `Ran 209 tests ... OK`.
...
- The clean-room handoff bundle contains 218 payload files and verifies its
