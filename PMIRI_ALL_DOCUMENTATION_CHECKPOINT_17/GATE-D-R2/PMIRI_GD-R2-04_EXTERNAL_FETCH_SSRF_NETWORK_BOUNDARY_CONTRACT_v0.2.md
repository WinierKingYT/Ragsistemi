...
The profile serializes the accepted scheme as `HTTPS` and defines destination
identity as scheme plus authority only. The exact request path/query is a
separate request-target binding: empty path becomes `/`, RFC3986 dot-segments
are removed, unreserved characters are decoded, reserved semantics are
preserved, percent-encoding hex is uppercase, query parameter order is
preserved and an absent query remains absent. Path/query are not silently
folded into the authority identity; the same request-target fingerprint must
be used by policy evaluation and the actual connector request.
...
public. If every returned address is denied, the typed connection record uses
`selected_target=null`, `resolution_set_status=DENIED_MIXED` and
`binding_status=DENIED` or `STALE`. This `NO_APPROVED_SELECTION` branch is
required and cannot be converted into an outbound allow result.
...
The corrected lifecycle schema revision is `0.3`. Every non-`NOT_APPLICABLE`
...
outbound_network_decision:
  ...
  selected_target_resolution_ref: <exact approved resolution entry or null>
  selected_target_ip: <exact approved IP or null>
  selected_target_family: IPv4|IPv6|null
  resolution_set_status: ALL_ALLOWLISTED_PUBLIC|DENIED_MIXED
  connection_binding_status: VALIDATED|DENIED|STALE
  final_revalidation_event_sequence: <last event sequence>
  final_revalidation_result: MATCHED|MISMATCHED|DENIED
  ...
  fetched_content_lifecycle_ref: <null for provider_call, required otherwise>
  fetched_content_lifecycle_fingerprint: <null for provider_call, required otherwise>
  fetched_content_origin_binding_ref: <null for provider_call, required otherwise>
  fetched_content_destination_binding_ref: <null for provider_call, required otherwise>
  fetched_content_admission_state: <null for provider_call, required otherwise>
  fetched_content_retrieval_visibility: <null for provider_call, required otherwise>
  fetched_content_history_terminal_state: <null for provider_call, required otherwise>
  fetched_content_terminal_response_fingerprint: <null for provider_call, required otherwise>
...
The corrected network schemas use schema revision `0.3`. The connection
...
`tls_identity_status=MATCHED`, `resolution_set_status=ALL_ALLOWLISTED_PUBLIC`,
`connection_binding_status=VALIDATED` and
`final_revalidation_result=MATCHED`; a mismatched TLS identity, denied/stale
binding or non-matched final event cannot reach an allow action. The terminal `content_lifecycle` is required on every network
...
The resolved lifecycle's operation, purpose, state, visibility, origin,
destination, history terminal state and terminal response fingerprint must
equal the outbound values. `terminal_response_fingerprint` must equal the
lifecycle `response_fingerprint`. `provider_call` is the only
