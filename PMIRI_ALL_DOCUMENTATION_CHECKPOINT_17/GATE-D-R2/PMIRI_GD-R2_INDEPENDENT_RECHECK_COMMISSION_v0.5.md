...
# Independent Recheck Commission v0.5
...
- per-dimension provider-policy `observed_value` tagged union and dimension
  equality;
- closed capability-observation key/value union and observation fingerprint
  binding;
- all-denied DNS `NO_APPROVED_SELECTION` representation;
- outbound validated-binding/final-revalidation requirements;
- exact lifecycle operation/purpose/origin/destination/admission/visibility/
  terminal-response equality;
- canonical scheme/path/query request-target rules and refreshed profile hash;
- trust-decision `content_lifecycle=NOT_APPLICABLE` closure;
- all previous IR-D2-01..30 findings, Gate-C lineage bindings and deferred
  D3 boundary.
...
evidence and required action, reassessment of IR-D2-01..30, final verdict and
