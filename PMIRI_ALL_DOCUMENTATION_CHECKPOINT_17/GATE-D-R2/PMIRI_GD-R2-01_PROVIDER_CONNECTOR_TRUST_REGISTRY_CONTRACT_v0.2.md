...
The trust-decision schema revision is `0.3`. Trust decisions are not
content-bearing fetch lifecycles; `content_lifecycle` is therefore fixed to
`NOT_APPLICABLE` and cannot be used to smuggle a fetched-content state into a
trust result.
